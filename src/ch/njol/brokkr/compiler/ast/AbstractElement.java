...
public abstract class AbstractElement<E extends Element> implements Element {
	...
	public @Nullable Element parent = null;
	
	@Override
	public void setParent(final @Nullable Element parent) {
		assert parent != this;
	...
	protected final @Nullable InterpretedNativeTypeDefinition getStandardType(final String module, final String type) {
		return BrokkrFile.getStandardType(this, module, type);
	}
	...
	private final List<ElementPart> parts = new ArrayList<>();
...
//		}
	...
	public static interface ElementProcessor<T extends Element> {
		T process() throws ParseException;
	}
	
	// TODO optimise if nothing matched? (how?)
	protected void until(final AbstractElement.VoidProcessor lambda, final char until, final boolean allowEmpty) {
...
//					expectedFatal("what?", start, length);
	...
	protected void repeatUntil(final AbstractElement.VoidProcessor lambda, final char until, final boolean allowEmpty) {
		if (allowEmpty && try_(until)) {
			return;
		}
		startGuard(until);
	...
	protected void repeatUntilEnd(final AbstractElement.VoidProcessor lambda) {
		while (peekNext() != null) {
			final int s = in.getTokenOffset();
			try {
				lambda.process();
	...
	protected void untilEnd(final AbstractElement.VoidProcessor lambda) {
		try {
			lambda.process();
		} catch (final ParseException ex) {}
		if (!in.isAfterEnd()) {
	...
	protected void oneGroup(final char start, final AbstractElement.VoidProcessor lambda, final char end) throws ParseException {
		one(start);
		until(lambda, end, true);
	}
	
	protected void oneRepeatingGroup(final char start, final AbstractElement.VoidProcessor lambda, final char end) throws ParseException {
		one(start);
		repeatUntil(lambda, end, true);
	}
	
	protected boolean tryGroup(final char start, final AbstractElement.VoidProcessor lambda, final char end) {
		if (try_(start)) {
			until(lambda, end, true);
			return true;
		}
	...
	protected boolean tryRepeatingGroup(final char start, final AbstractElement.VoidProcessor lambda, final char end) {
		if (try_(start)) {
			repeatUntil(lambda, end, true);
			return true;
		}
	...
	/**
	 * Parses several blocks in an unordered fashion. make sure the blocks don't consume any tokens if they have not been parsed (i.e. use {@link #try_(String)} or similar)
	 */
	protected void unordered(final AbstractElement.VoidProcessor... lambdas) {
		@SuppressWarnings("null")
		final List<AbstractElement.VoidProcessor> notDone = new ArrayList<>(Arrays.asList(lambdas));
		int start;
		outer: while (true) {
			start = in.getTokenOffset();
...
//			expectedFatal("one of "+Arrays.toString(symbolsAndWords), start, 1);
	...
	@SuppressWarnings("unchecked")
	protected final <R extends Element, T extends AbstractElement<R>> R one(final T t) throws ParseException {
//			final int start = in.getTokenOffset();
		@Nullable
		R r;
		try {
...
//			assert !r.fatalParseErrors().isEmpty() || r != t || in.getTokenOffset() > start : t + " / " + in + " :: " + ((AbstractElement<?>) t).possibleParseErrors;
	...
	protected final <R extends Element, T extends AbstractElement<R>> R one(final Class<T> type) throws ParseException {
		try {
